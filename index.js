...
async function main(type, message) {
  ...
  for (var category in config.categories) {
    ...
    for (var server of config.categories[category]) {
      ...
      if (server.game === "fivem") { playersnum = data.length; maxplayers = server.maxplayers; } else if (server.game === "minecraft") { playersnum = data.onlinePlayers; maxplayers = data.maxPlayers; } else { playersnum = data.playersnum; maxplayers = data.maxplayers; }
      if (playersnum === undefined || maxplayers === undefined) { playersnum = "∞"; maxplayers = "∞" }
...
/* Minecraft */ async function mc(ip, port) {  await mcquery.status(ip, { port: port, timeout: 1000 }) }
/* Commands */
